  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-lg w-full border border-gray-200 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-gray-900 space-y-5 p-6">
        ...
        {(() => {
          const evalMetrics = analysisData?.evaluation?.metrics || {}
          const displayCpu = evalMetrics.cpu_utilization !== undefined ? evalMetrics.cpu_utilization : workload.cpu
          const displayNet = evalMetrics.network_kbps !== undefined ? evalMetrics.network_kbps : workload.network_kbps
          const displaySockets = evalMetrics.active_connections !== undefined ? evalMetrics.active_connections : workload.active_connections
          const displayIops = evalMetrics.disk_io_iops !== undefined ? evalMetrics.disk_io_iops : workload.iops
          const mlClass = analysisData?.evaluation?.ml_classification || (workload.idle_confidence > 50 ? 'TRUE_IDLE' : 'ACTIVE_BUSY')
          const mlConf = analysisData?.evaluation?.ml_confidence !== undefined ? Math.round(analysisData.evaluation.ml_confidence * 100) : workload.idle_confidence

          return (
            <>
              <div className="space-y-2">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-700 block">
                  5-Dimensional Real-Time Telemetry (Backend Evaluated):
                </span>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                    <span className="text-[10px] text-gray-500 block">CPU Usage</span>
                    <span className="text-sm font-extrabold text-gray-900 font-mono mt-0.5">{displayCpu}%</span>
                    <span className="text-[9px] text-emerald-600 font-semibold block">&lt; 2.5% idle limit</span>
                  </div>

                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                    <span className="text-[10px] text-gray-500 block">Network I/O</span>
                    <span className="text-sm font-extrabold text-gray-900 font-mono mt-0.5">{displayNet} KB/s</span>
                  ...
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <span className="text-sm font-extrabold text-gray-900 font-mono mt-0.5">{displaySockets}</span>
                  ...
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <span className="text-sm font-extrabold text-gray-900 font-mono mt-0.5">{displayIops}</span>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-50/80 via-white to-emerald-50/80 border border-blue-200 space-y-2.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 font-medium">Isolation Forest ML:</span>
                  <span className="font-mono font-bold text-emerald-700 text-sm">{mlConf}% {mlClass}</span>
                </div>
