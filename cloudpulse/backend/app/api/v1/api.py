...
from app.api.v1.endpoints import resources, ghost_resources, policies, hooks, analytics, ml, edge, vault, events

api_router = APIRouter()

api_router.include_router(resources.router, prefix="/resources", tags=["Cloud Resources"])
api_router.include_router(edge.router, prefix="/edge", tags=["VEGA Edge Controller & Telemetry"])
api_router.include_router(vault.router, prefix="/vault", tags=["Cryptographic State Vault"])
api_router.include_router(events.router, prefix="/events", tags=["Audit & Event Ledger"])
